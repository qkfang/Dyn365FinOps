<#
.SYNOPSIS
    Import this module to get functions to handle file downloads from the asset library.
    
.DESCRIPTION
    This script can be imported to enable cmdlets to get file asset's details and download from blob storage.
...
<#
.SYNOPSIS
    Make REST API call to Dynamics Lifecycle Services to get a file asset
#>
function Get-LCSFileAsset
{
    ...
    $createFileAssetUri = "$LCSAPI/box/fileasset/GetFileAsset/$($ProjectId)?assetId=$AssetId"

    $request = New-JsonRequestMessage -Uri $createFileAssetUri -Content $fileAssetJson -BearerTokenHeader $BearerTokenHeader -HttpMethod ([System.Net.Http.HttpMethod]::Get)
    ...
    if ($result.StatusCode -ne [System.Net.HttpStatusCode]::OK)
    {
        ...
        {
            throw "Error getting file asset: '$($asset.Message)'"
        }
        else
        {
    ...
    if (!($asset.Id))
    {
        ...
        {
            throw "Unknown error getting file asset"
        }
    }
...
<#
.SYNOPSIS
    Make REST API call to Dynamics Lifecycle Services to find a file asset, and return the list of file assets found
#>
function Find-LCSFileAsset
{
    [Cmdletbinding()]
    param(
        [Parameter(Mandatory=$true, HelpMessage="Azure Active Directory authorization bearer token header")]
        ...
        [Parameter(Mandatory=$true, HelpMessage="Dynamics Lifecycle Services project ID")]
        [int]$ProjectId,
        [Parameter(Mandatory=$true, HelpMessage="Asset type to find")]
        [int]$FileType,
        [Parameter(Mandatory=$false, HelpMessage="Name of the assets to find")]
        [string]$AssetName,
        [Parameter(Mandatory=$false, HelpMessage="Optional version of the assets to find")]
        [string]$AssetVersion,
        [Parameter(Mandatory=$false, HelpMessage="LCS API URL")]
        [string]$LCSAPI = "https://lcsapi.lcs.dynamics.com"
    )
    ...
    $createFileAssetUri = "$LCSAPI/box/fileasset/GetAssets/$($ProjectId)?fileType=$($FileType)"
    ...
    if ($result.StatusCode -ne [System.Net.HttpStatusCode]::OK)
    {
        ...
        if (($asset) -and ($asset.Message))
        {
            throw "Error finding file assets: '$($asset.Message)'"
        }
        else
        {
            throw "API Call returned $($result.StatusCode): $($result.ReasonPhrase)"
    ...
    $assets = Get-AsyncResult -task $result.Content.ReadAsStringAsync() | ConvertFrom-Json

    if ($AssetName)
    {
        $assets = $assets | Where-Object { $_.Name -like $AssetName }
    }
...
<#
.SYNOPSIS
    Download file from blob storage URL returned by the Dynamics Lifecycle Services File Asset API
#>
function Get-FileFromBlob
{
    [Cmdletbinding()]
    param(
        [Parameter(Mandatory=$true, HelpMessage="Block blob URL to download from")]
        [System.Uri]$BlockBlobUri,
        [Parameter(Mandatory=$true, HelpMessage="Full file path to download contents into")]
        [string]$FilePath
    )
    ...
    $downloadResult = Get-AsyncResult -task $cloudblob.DownloadToFileAsync([System.String]$FilePath, [System.IO.FileMode]::CreateNew)
}

# SIG # Begin signature block
# MIIoDwYJKoZIhvcNAQcCoIIoADCCJ/wCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBVze4Shb9ABSj5
# wm8vzDPfsoVVq5QfDv2Wz25Jt4dIEqCCDXYwggX0MIID3KADAgECAhMzAAAEBGx0
...
# IJamTc0RoaY/9tHBIBExNyezzNp9S+vPBxJBpEdWSC8aMEIGCisGAQQBgjcCAQwx
# NDAyoBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29m
# dC5jb20wDQYJKoZIhvcNAQEBBQAEggEAjHHhlJf04rpbtqaVBSh2+1zs20ZBsh8N
# f1fP5WU1gjuiKNcg9AB9xfVsel6Bf5ExDdXO8DyTh1UhuWDqw8jLVRhAgtiQf/sO
# A162FBMYB0zqfcLlee+d7WtPYoeSKUxYKo8wavDv6jlu2DUhhzT3cKoSndKUpzer
# 8eH4tA1VecUIz2WrCxK7Nrr1k51/a159/BDa/jXVWWy6Si9dMsFOWuKticIioHHc
# 72LB5ekFovKzGSkPBhBSeKiWf61sIpYom+Zbi5pEuXCfzlFzJtX5bqmBf0YPgsq2
# FVu97vhH/Cc8Ez8TuyZbRIgcyTz9AeAkpKwIb6dl5/31dDgAaB38O6GCF5cwgheT
...
# BAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCDFneB+NPUCjubkWha7NM47tRcMVs/0
# 46ALykB6twlJvQIGZ/g/gJSHGBMyMDI1MDQxODAwMTk0OS41MzFaMASAAgH0oIHR
...
# Y3Jvc29mdCBUaW1lLVN0YW1wIFBDQSAyMDEwMA0GCSqGSIb3DQEBCwUAAgUA66v3
# 5jAiGA8yMDI1MDQxNzIxNTc1OFoYDzIwMjUwNDE4MjE1NzU4WjB3MD0GCisGAQQB
# hFkKBAExLzAtMAoCBQDrq/fmAgEAMAoCAQACAg/UAgH/MAcCAQACAhL7MAoCBQDr
# rUlmAgEAMDYGCisGAQQBhFkKBAIxKDAmMAwGCisGAQQBhFkKAwKgCjAIAgEAAgMH
# oSChCjAIAgEAAgMBhqAwDQYJKoZIhvcNAQELBQADggEBAA+yd1QSLktmV1vap/pd
# arAQ+DXVaqnLVwepb7Mb+pm+TrIVwqliomfDRCMv+gNpOKvXxPwGmf2a/nHfOLTg
# Z1kHsc2OT0LJi/dDcsxZusoTn6tmb2Tf+tGF0dhvx/S+fFe0ZgmDmFzPc9fKpytt
# 50YiVq/uoo3CmIEwXhwsHCFND3JZPaDVDeIR4bC9GdaYqaqOcm2Nxyg9B/5IaaU/
# 2BoKbkXJWuhBH3PTX6dzYhOC9F1Ejcr9iXayp4Mlvzm1NLuapLBfyeMnkn0dxYBu
# yGPKpbreH19zdFfe9ma8Fb4FSCVHvlGsq1K0IoOW2F5QkZO3q7boX2ksHfWn8fDE
# VdExggQNMIIECQIBATCBkzB8MQswCQYDVQQGEwJVUzETMBEGA1UECBMKV2FzaGlu
# Z3RvbjEQMA4GA1UEBxMHUmVkbW9uZDEeMBwGA1UEChMVTWljcm9zb2Z0IENvcnBv
# cmF0aW9uMSYwJAYDVQQDEx1NaWNyb3NvZnQgVGltZS1TdGFtcCBQQ0EgMjAxMAIT
# MwAAAgsRnVYpkvm/hQABAAACCzANBglghkgBZQMEAgEFAKCCAUowGgYJKoZIhvcN
# AQkDMQ0GCyqGSIb3DQEJEAEEMC8GCSqGSIb3DQEJBDEiBCBe9JSTYcAbmp6BD4vU
# aB5FY11fbWc8Y8KioHll2nCIDDCB+gYLKoZIhvcNAQkQAi8xgeowgecwgeQwgb0E
...
# S2mMemsbLcidDhqz/xfD7dYySRrzYk+gnDp1FGKMdLUwDQYJKoZIhvcNAQELBQAE
# ggIAQGvK3+JCnbZkAnH9fVRI5wUFepx/R39XPVS6EbmxfCiJ1nloRdpimR1xNI9Y
# 5creLKMG3ox8DYeFkw55QJgWijC/9YMg7jVudUdt5B67f4G2Y2VRY9o3Ezp4ADeK
# BQ8Xe/OmXNskxLduCeu6Yvkd+clKtwt8WE+vJqipjwVQr9LRRk2X/SlLX7/1n2N+
# pjmQYiAOvLqoDSia6TgihCECaIziBAO9cRJtZR6N6gDMvUumheokgDJ4mH7P/5NH
# GU5XqlVz5GumELQ5qMi4C2IwypttJMeETAb8hMZcHx9huVlQqsYSXWPl2ITVfzqP
# sh8JklarZ9fPqAFwwSUIDXDs9SaJ+oC7ji+h0wkcKPbomSqFBUwBzZ4eeHjxgq/N
# a10PoIAyMs3H3fv5JNQ5fihzgOrL20a4BxO1FN7aYGRsi9+nVg9kdhJWz5l0cGl3
# D4ngqVZoz2ALOO2b4O0NS0QB3ul91sqNn4Wsfr2/pqDvQY4fu/TbP7Pgnk9p5LfP
# ZyVOH7UopGewG9TGlxhb4i6j5KrNDfBlRqUBAfVkUYMlZ0DWbm2blmmRrjwFxg2I
# ipt4SB7/KL1ZK5EZ/A0MmIKjIxW/TW/MHBy6OXIMgiUJBf11ChEqY9GjoEVvP4k4
# pP+UqRoHNd2lREl08xoIY4Tq60QNSlgcmXf1NXjPSDPXf8o=
